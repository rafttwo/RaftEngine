...
namespace RaftEngine {
	...
	int Window::init()
	{
        LOG_INFO("Creating window {0} windth size {1}x{2}", m_data.title, m_data.width, m_data.height);
        ...
        if (!m_pWindow)
        {
            LOG_CRITICAL("Failed create window '{0}' windth size {1}x{2}", m_data.title, m_data.width, m_data.height);
            glfwTerminate();
            return -2;
        }
